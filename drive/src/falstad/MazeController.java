...
public class MazeController implements Order {
	...
	//public static MazePanel titlePanel;
	public static MazeApplication app;
	...
	private void switchToGeneratingScreen() {
		assert state == StateGUI.STATE_TITLE : "MazeController.switchToGeneratingScreen: unexpected current state " + state ;
		// switch state and update screen
		app.remove(MazePanel.titlePanel);
		app.add(panel);
		app.validate();
		setState(StateGUI.STATE_GENERATING);
		percentdone = 0;
		...
		while (it.hasNext())
		{
			...
			if (panel.getBufferGraphics() == null) {
				...
				if (state==StateGUI.STATE_FINISH && v instanceof MazeView) {
					...
					v2.steps = steps;
					v2.redraw(panel, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle) ;
				}
				else v.redraw(panel, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle) ;
			}	
		}
