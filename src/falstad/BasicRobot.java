...
public class BasicRobot implements Robot {
	...
	public BasicRobot() {
		...
		//currentPosition = mazeControl.getCurrentPosition();
	}
	...
	/**Rotate calls the KeyDown method of MazeController
	 * It has a switch statement: one case Left, which passes the key left, one case
	 * Right which passes the key right, and one case Around which passes the key right twice.
	...
	public void move(int distance, boolean manual) {
		for (int i = 0; i < distance; i++) {
			...
			//if (this.isAtExit()) this.resetRobot();
		}
	...
	public int[] getCurrentPosition() throws Exception {
		if (!mazeControl.getMazeConfiguration().isValidPosition(mazeControl.getCurrentPosition()[0], mazeControl.getCurrentPosition()[1]))
			throw new Exception();
		return mazeControl.getCurrentPosition();
	...
	public boolean isAtExit() {
		return mazeControl.getMazeConfiguration()
				.getMazecells().isExitPosition(mazeControl.getCurrentPosition()[0], mazeControl.getCurrentPosition()[1]);
	}
	...
	public int distanceToObstacle(Direction direction) {
		...
		if(cd==CardinalDirection.North||cd==CardinalDirection.South) cd=cd.oppositeDirection();
		int[] checkedPosition = null;
		try {
			checkedPosition = this.getCurrentPosition();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		int[] differenceInPositions = cd.getDirection();
		//differenceInPositions[1]*=-1;
		int count = 0;
		...
		while (!(mazeControl.getMazeConfiguration().hasWall(checkedPosition[0], checkedPosition[1], cd))){
			...
			if(!mazeControl.getMazeConfiguration().isValidPosition(checkedPosition[0], checkedPosition[1])) return count;
			count++;
		}
		//}
		//catch(ArrayIndexOutOfBoundsException e) {
		//	//TODO: find out why this happens. For now, a band-aid will do
		//	return --count;
		//}
	...
	protected Turn translateCardinal(CardinalDirection dir) {
		
		if (this.currentDirection.rotateCounterClockwise() == dir) return Turn.LEFT;
		
		if (this.currentDirection.rotateClockwise() == dir) return Turn.RIGHT;
		
		if (this.currentDirection.oppositeDirection() == dir) return Turn.AROUND;
	...
	public boolean isInsideRoom() throws UnsupportedOperationException {
		...
		return mazeControl.getMazeConfiguration()
				.getMazecells().isInRoom(mazeControl.getCurrentPosition()[0], mazeControl.getCurrentPosition()[1]);
	}
